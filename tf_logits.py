...
def compute_mfcc(audio, **kwargs):
    ...
    audio = tf.concat((audio[:, :1], audio[:, 1:] - 0.97*audio[:, :-1],
                      np.zeros((batch_size, 512), dtype=np.float32)), 1)

    # 2. windowing into frames of 512 samples, overlapping
    windowed = tf.stack([audio[:, i:i+512]
                        for i in range(0, size-320, 320)], 1)
    ...
    feat = tf.matmul(ffted, np.array(
        [filters]*batch_size, dtype=np.float32))+np.finfo(float).eps
    ...
    feat = tf.concat(
        (tf.reshape(tf.math.log(energy), (-1, width, 1)), feat[:, :, 1:]), axis=2)
...
def get_logits(new_input, length, first=[]):
    ...
    empty_context = tf.zeros((batch_size, 9, 26), dtype=tf.float32)
    ...
    features = tf.stack([features[:, i:i+19*26]
                        for i in range(0, features.shape[1]-19*26+1, 26)], 1)
